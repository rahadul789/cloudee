...
function getOwnerAutoCancelSettings(content: Record<string, any>) {
  ...
  return {
    ...
    prepLateGraceMinutes:
      typeof dispatch.prepLateGraceMinutes === "number" ? dispatch.prepLateGraceMinutes : 5
  }
}

type OwnerAutomationSettings = ReturnType<typeof getOwnerAutoCancelSettings>

function ownerAutomationScopeKey(serviceAreaSnapshot?: Record<string, any> | null) {
  ...
  if (status === "New") {
    ...
    const lateBySeconds =
      createdAt > 0 && !Number.isNaN(createdAt)
        ? Math.max(0, Math.ceil((now - createdAt) / 1000) - settings.ownerAcceptanceTimeoutMinutes * 60)
        : 0
    if (lateBySeconds > 0) {
      return {
        isLate: true,
...
async function resolveRecommendedMenuItemIds(params: {
  ...
  const currentItemId = params.currentItemId ? String(params.currentItemId) : ""
  const uniqueIds = [
    ...new Set(
      (params.itemIds ?? [])
        .map((id) => id.trim())
        .filter((id) => id && mongoose.Types.ObjectId.isValid(id) && id !== currentItemId)
    )
  ]
...
export async function listOrders(params: {
  ...
  const contentRecord = content as Record<string, any>
  const automationSettingsCache = new Map<string, Promise<OwnerAutomationSettings>>()
...
export async function transitionOrder(params: {
  ...
  const ownerOrderObject = decorateOwnerOrderAutomation(
    applyOwnerOrderPrivacy(orderObject, getOwnerAppSettings(responseContentRecord)),
    await getOwnerAutomationSettings(
